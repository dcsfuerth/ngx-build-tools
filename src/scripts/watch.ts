...
import { buildFactory, RollupConfig, yargsFailFn } from './../utils/build';
...
const args = yargs
  ...
  .describe('cache', 'use the rollup caching mechanism (breaks with some plugins)')
  .default('ignored', '(^|[\\/\\\\])\\..')
  .alias('ignored', 'i')
...
const config: RollupConfig = require(path.resolve(configFile));
